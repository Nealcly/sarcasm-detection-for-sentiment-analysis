...
class multi_layer_BiLSTM(object):
    """
    LSTM model
    """
    ...
    def __init__(
        ...
        with tf.device('/cpu:0'), tf.name_scope("Embedding"):
            self.W = tf.Variable(tf.random_uniform([vocab_size, input_embedding_size], -0.1, 0.1), trainable=True,
                                 name="W", dtype=tf.float32)
            self.embedding_placeholder = tf.placeholder(tf.float32, [vocab_size, sequence_length],
        ...
        with tf.name_scope("Model"):
            ...
            #mlstm_cell = rnn.MultiRNNCell(cells = [lstm_cell] * num_layers)
            #, state_is_tuple=True
            lstm_fw_cell = []
            for i in range(num_layers):
                ...
                # Add dropout
                lstm_cell = tf.contrib.rnn.DropoutWrapper(lstm_cell, output_keep_prob=self.dropout_keep_prob)
                lstm_fw_cell.append(lstm_cell)
            lstm_fw_cell = rnn.MultiRNNCell(cells = lstm_fw_cell)
            ...
            for i in range(num_layers):
                ...
                # Add dropout
                lstm_cell = tf.contrib.rnn.DropoutWrapper(lstm_cell, output_keep_prob=self.dropout_keep_prob)
                lstm_bw_cell.append(lstm_cell)
            lstm_bw_cell = rnn.MultiRNNCell(cells = lstm_bw_cell)
